...
PSCError Widget::dispose(WidgetPool& widgetPool, Widget* pWidget)
{
...
PSCError Widget::getError() const
{
    PSCErrorCollector error = m_error.get();
    ...
    for (std::size_t idx = 0U; idx < numChildren(); ++idx)
    {
        ...
        // coverity[stack_use_unknown]
        PSCError childError = pChild->getError();
        error = childError;
    }
